...
Fitnesses::BGFitness::BGFitness(QJsonObject const &a_config)
{
  ...
  {
    //Logger->trace("activeFitnessList[{}]:{}", i, activeFitnessList[i].toStdString().c_str());
...
void Fitnesses::BGFitness::process(std::vector<_postData> &_data)
{
  ...
  Logger->trace("m_errors.tpError:{}", m_errors.tpError);
...
{
  //Logger->trace("Fitnesses::BGFitness::endProcess");
  Logger->trace("m_errors.fnError:{}", m_errors.fnError);
  Logger->trace("m_errors.fpError:{}", m_errors.fpError);
  Logger->trace("m_errors.tnError:{}", m_errors.tnError);
  Logger->trace("m_errors.tpError:{}", m_errors.tpError);
  ...
  fs.fitnessTime = 0;
  //Logger->trace("fitnessTime:{}", fs.fitnessTime);
  ...
  _data[0].fs = fs;
  //return fs;
  //Logger->trace("_data[0].fs.tpError:{}", _data[0].fs.Accuracy);
 // Logger->trace("_data[0].fs.fpError:{}", _data[0].fs.Recall);
 // Logger->trace("_data[0].fs.tnError:{}", _data[0].fs.FMeasure);
 // Logger->trace("_data[0].fs.tpError:{}", _data[0].fs.Precision);
